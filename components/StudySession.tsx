...
interface StudySessionProps {
  ...
  onExit: () => void;
  onReview: (phraseId: string, isCorrect: boolean) => void; // Updated signature
  onTimeUpdate: (seconds: number) => void; 
}

export const StudySession: React.FC<StudySessionProps> = ({ deck, onUpdateDeck, onExit, onReview, onTimeUpdate }) => {
  ...
  const renderStatsOverlay = () => {
    ...
    return (
      <div className="absolute top-14 right-4 bg-white shadow-xl rounded-xl p-4 z-40 border border-slate-100 min-w-[240px] animate-in slide-in-from-top-2">
    ...
    return (
      <div className="fixed inset-0 bg-slate-50 flex flex-col items-center justify-center p-6 text-center z-50">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full">
          ...
          <Button onClick={onExit} fullWidth>返回主页</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 bg-slate-100 z-50 flex flex-col relative overflow-hidden h-full">
      ...
      {(showQueue || showStats) && (
        <div 
          ...
          onClick={() => { setShowQueue(false); setShowStats(false); }}
        />
      )}
      ...
      {/* Compact Header */}
      <div className="bg-white shadow-sm px-3 py-2 flex justify-between items-center z-10 shrink-0 h-12">
